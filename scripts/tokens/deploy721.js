...
const contract_name         = "TEMPLETEERC721";
...
async function create_token_info(name, symbol, address) {
    return {
        "name":     name,
        "symbol":   symbol,
        "address":  address
...
async function bak_conf() {
    let pathname = violas.erc721_tokens_conf;
    let mark = logger.date_format("", "", "");
    let filename = path.basename(pathname)
    let new_pathname = bak_path + mark + "_" + filename;

    const {old_tokens}  = require(violas.erc721_tokens_conf);
...
async function deploy(name, symbol) {
    ...
    const dp = await cf.deploy();
    await dp.deployed();
    logger.info(name + " deployed to: " + dp.address);

    token = await create_token_info(name, symbol, dp.address);
    await update_conf(violas.erc721_tokens_conf, token);
